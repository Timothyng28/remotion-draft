      ...
      {data.isQuestionNode && (
        <div
          ...
          style={{
            ...
            boxShadow:
              "0 0 20px rgba(251, 191, 36, 0.9), inset 0 0 10px rgba(251, 191, 36, 0.3)",
            transform: "scale(1.3)",
          }}
        />
      )}
    ...
    for (let level = startLevel; level <= endLevel; level++) {
      ...
      nodesAtLevel.forEach((node, indexAtLevel) => {
        ...
        if (totalNodesAtLevel > 1) {
          ...
          const offset =
            (indexAtLevel - (totalNodesAtLevel - 1) / 2) * branchYOffset;
          yPos = centerY + offset;
        }
        ...
        if (node.parentId && nodePositions.has(node.parentId)) {
          ...
          const colorIndex = node.branchIndex % nodeColors.length;
          const edgeColor = isOnCurrentPath
  ...
  return (
    <>
      ...
      >
        ...
        {/* Tree visualization */}
        <div className="w-full h-full cursor-pointer relative tree-visualizer-mini dot-bg">
          <style>{`
